...
There are a couple of important things to note here. The first is that with a data frame we can have different types of data. For example try converting our td data frame to a matrix using the `as.matrix()` function. 
```{r, message = T, eval = F}
as.matrix(td)
```
What happens when you do this? Why do you think that is? 
...
Now that we're familiar with R we can begin using it to explore our own data. We'll use the `td` object that we created a moment ago. This is a data set we collected in Siberia that contains measurments of permafrost active layer thickness. 
